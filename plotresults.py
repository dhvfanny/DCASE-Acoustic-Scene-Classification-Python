...
def plot_cm(cm, targets, title="Confusion Matrix", cmap=plt.cm.Blues, norm=True):
    if(norm):
...
for f in glob.glob('result*.pkl'):
   ...
   print clf_name
   plot_cm(res['tot_cm'], res['result']['class_wise_accuracy'].keys(), title='Confusion Matrix - '+clf_name)
