...
#    print feature_matrix.shape
    if statistics:
        return {
            'feat': feature_matrix,
            'stat': {
                'mean': feature_matrix, #numpy.mean(feature_matrix, axis=0),
                'std': numpy.zeros_like(feature_matrix),#(numpy.std(feature_matrix, axis=0),
...
def feature_extraction_traps_merger(audio_filename_with_path, statistics=True):
    print audio_filename_with_path
    feature_matrix =  pickle.load(open(audio_filename_with_path, "rb"))
    feature_matrix = feature_matrix.astype(numpy.float)[None]
    ...
    if statistics:
		...
		'mean': numpy.mean(feature_matrix,axis=0),
            ...
            #    'mean': feature_matrix, #numpy.mean(feature_matrix, axis=0),
             #   'std': numpy.zeros_like(feature_matrix),#(numpy.std(feature_matrix, axis=0),
#		'N': feature_matrix.shape[0],
               # 'S1': feature_matrix,#numpy.sum(feature_matrix, axis=0),
               # 'S2': feature_matrix**2,#numpy.sum(feature_matrix ** 2, axis=0),
            }
        }
    else:
        return {
    ...
    if statistics:
        return {
            ...
            'stat': {
                ...
                'N': feature_matrix.shape[0],
                'S1': numpy.sum(feature_matrix),
                'S2': numpy.sum(feature_matrix ** 2),
            }
        }
    else:
        return {
