...
def main(argv):
    ...
    params = load_parameters('svc.yaml')
...
def do_system_training(dataset, model_path, feature_normalizer_path, feature_path, classifier_params,
                       dataset_evaluation_mode='folds', classifier_method='svc', overwrite=False):
    """System training
...
#                    model_container['models'][label] = mixture.GMM(**classifier_params).fit(data[label])
                    ...
                    raise ValueError("Unknown classifier method ["+classifier_method+"]")
	    if classifier_method == 'svc':
                model_container['models'] = SVC(probability=True).fit(tot_data['x'], tot_data['y'])
            # Save models
        ...
        if not os.path.isfile(current_result_file) or overwrite:
            ...
            for file_id, item in enumerate(dataset.test(fold)):
                ...
                if classifier_method == 'svc':
                    ...
                    current_result = dataset.scene_labels[do_classification_svc(feature_data, model_container)]
                else:
                    raise ValueError("Unknown classifier method ["+classifier_method+"]")
...
def do_classification_svc(feature_data, model_container):
