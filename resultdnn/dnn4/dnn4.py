...
def main(argv):
    ...
    params = load_parameters('dnn4.yaml')
...
def do_system_training(dataset, model_path, feature_normalizer_path, feature_path, classifier_params,
                       dataset_evaluation_mode='folds', classifier_method='dnn4', overwrite=False):
    """System training
    ...
    for fold in dataset.folds(mode=dataset_evaluation_mode):
        ...
        if not os.path.isfile(current_result_file) or overwrite:
            ...
            for file_id, item in enumerate(dataset.test(fold)):
                ...
                if classifier_method == 'dnn4':
                    current_result = dataset.scene_labels[do_classification_dnn4(feature_data, model_container)]
                else:
                    raise ValueError("Unknown classifier method ["+classifier_method+"]")
...
def do_classification_dnn4(feature_data, model_container):
    ...
    model_clf =  skflow.TensorFlowEstimator.restore('dnn4/dnn4model1');
