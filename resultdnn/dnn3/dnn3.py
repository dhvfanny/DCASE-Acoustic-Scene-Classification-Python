...
def main(argv):
    ...
    params = load_parameters('dnn3.yaml')
...
def do_system_training(dataset, model_path, feature_normalizer_path, feature_path, classifier_params,
                       dataset_evaluation_mode='folds', classifier_method='dnn3', overwrite=False):
    """System training
...
#                    model_container['models'][label] = mixture.GMM(**classifier_params).fit(data[label])
                    ...
                    raise ValueError("Unknown classifier method ["+classifier_method+"]")
	    clf = skflow.TensorFlowDNNClassifier(hidden_units=[10,500,500,500,10], steps=1000, learning_rate=0.05,batch_size=256, n_classes=10)
	    if classifier_method == 'dnn3':
                tot_data['y'] = le.fit_transform(tot_data['y'])
                clf.fit(tot_data['x'], tot_data['y'])
                clf.save('dnn3/dnn3model1')
    ...
    for fold in dataset.folds(mode=dataset_evaluation_mode):
        ...
        if not os.path.isfile(current_result_file) or overwrite:
            ...
            for file_id, item in enumerate(dataset.test(fold)):
                ...
                if classifier_method == 'dnn3':
                    current_result = dataset.scene_labels[do_classification_dnn3(feature_data, model_container)]
                else:
                    raise ValueError("Unknown classifier method ["+classifier_method+"]")
...
def do_classification_dnn3(feature_data, model_container):
    ...
    model_clf =  skflow.TensorFlowEstimator.restore('dnn3/dnn3model1');
